import { useId, type CSSProperties } from "react";

import { Card } from "@/components/ui/card";
import { Skeleton } from "@/components/ui/skeleton";

import { BlogCardBorder } from "./blogCardBorder";

export default function BlogCardSkeleton() {
  const clipPathId = useId().replaceAll(":", "");

  return (
    <div
      className="relative h-63 w-40 md:h-82 md:w-full md:max-w-75.5"
      style={{ "--blog-card-clip": `url(#${clipPathId})` } as CSSProperties}
    >
      <div className="blog-card-shadow relative h-full w-full">
        <Card
          aria-hidden="true"
          className="blog-card-surface h-full w-full gap-0 overflow-hidden rounded-none py-0 ring-0"
        >
          <div className="p-2 md:p-3">
            <Skeleton className="h-25 w-36 rounded-lg md:h-45 md:w-full" />
          </div>

          <div className="px-2 pb-2 md:px-3 md:pb-3">
            <div className="h-18 space-y-3 md:h-12">
              <Skeleton className="h-4 w-full md:h-5" />
              <Skeleton className="h-4 w-4/5 md:h-5" />
            </div>
          </div>

          <div className="m-2 space-y-1 md:m-3">
            <div className="flex items-center gap-2">
              <Skeleton className="size-4 shrink-0 rounded-full md:size-5" />
              <Skeleton className="h-3 w-20" />
            </div>
            <div className="flex items-center gap-2">
              <Skeleton className="size-4 shrink-0 rounded-full md:size-5" />
              <Skeleton className="h-3 w-24" />
            </div>
          </div>
        </Card>

        <BlogCardBorder clipPathId={clipPathId} />
      </div>

      <Skeleton className="absolute bottom-0 left-0 z-20 size-11 rounded-lg border border-border bg-muted md:size-14" />
    </div>
  );
}
